...
const DiseasePlanPage = () => {
  ...
  const handleCalendarClick = () => {
    if (aiResponse) {
      const encodedData = encodeURIComponent(JSON.stringify(aiResponse));
      window.open(
        `/dashboard/krishibhavishya/predictions/disease/plan/calendar?aiResponse=${encodedData}`,
        "_blank"
      );
    } else {
      toast.error("No AI response available to pass to the calendar.");
    }
  };
      ...
      <ScrollArea className="h-[57vh] w-full border rounded-md p-4">
        {aiResponse && (
          <div className="space-y-8">
            {/* Treatment Plan */}
            <section>
              <h2 className="text-xl font-semibold mb-4 text-primary">Treatment Plan <Button className="ml-5" onClick={handleCalendarClick}>View Calendar</Button></h2>
